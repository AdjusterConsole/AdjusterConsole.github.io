...
function resrcCenter(x, y) {
	...
	const displayValue = window.getComputedStyle(resrcDiv).display;
	if (displayValue == "none" && !checkOpen()) {
		resrcDiv.style.display = "inline-block";
		if (y == 'p') {
			document.getElementById("policyDiv").classList.remove("hide");
		}
		if (y == 't') {
			document.getElementById("toolsDiv").classList.remove("hide");
		}
		document.getElementById('trackerMsg').innerText = "";
...
function whichRep() {
	...
	if (whichB && !whichA && !whichC) {
		inceptEst();
		return;
	}
	if (whichC && !whichA && !whichB) {
		comsoCompan('1');
		return;
	}
	if (whichA && whichB && !whichC) {
...
function showNote(noteID) {
	...
	const displayValue = window.getComputedStyle(binElem).height;
	if (displayValue === "0px") {
		if (noteOpen != "false") {
			closeonFly();
		}
...
function noteOutput(z) {
	...
	for (let i = 0; i < objectArr.length; i++) {
		...
		if (objectArr[i].notes == null) {
			objectArr[i].notes = "None";
		}
